...
get_peak_genes_assign <- function(gtf, peaks, qval = qval){
  ...
  ## Filter out peaks that are entirely within the genes. 
  ## Throw out all peaks entirely within the gene & not overlapping with exon1, 
  ...
  ## Coordinates of the start and end of gene
  ...
  ## Assign the peaks to the nearest upstream gene (look at the 5 closest)
  ...
  ## Find the closest gene among the top 5 that is upstream (min V16)
  ...
  # Only peaks information to be loaded into IGV
...
# Called peaks (AP2XII8  vs 4 controls)
